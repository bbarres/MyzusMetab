...
#to trash after confirmation by claire 8 april 2022

nf<-layout(matrix(c(1,1,1,1,2,3),3,2,byrow=TRUE))

datatemp<-dataMyCor[dataMyCor$ana_id %in% c("69","70"),]
temp.m1<-drm(nb_mtot/(nb_mtot+nb_vi)~dose,
             data=datatemp,curveid=synerg_id,
             fct=LN.3u())
temp0<-EDcomp(temp.m1,c(0.5,0.5),type="absolute")
...
datatemp<-dataMyCor[dataMyCor$ana_id %in% c("1855","1856"),]
temp.m1<-drm(nb_mtot/(nb_mtot+nb_vi)~dose,
             data=datatemp,curveid=synerg_id,
             fct=LN.3u())
temp0<-EDcomp(temp.m1,c(0.5,0.5),type="absolute")
